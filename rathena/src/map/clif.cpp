...
void clif_dropflooritem( const flooritem_data* fitem, bool canShowEffect ){
	...
	if( canShowEffect ){
		...
		if( dropEffect > 0 ){
			// Respeta si le pusiste un DropEffect manual en item_db.yml
			p.showdropeffect = 1;
...
void clif_selllist( const map_session_data& sd){
	...
	for( int32 i = 0, c = 0; i < MAX_INVENTORY; i++ ){
		...
		if( battle_config.rental_item_novalue && sd.inventory.u.items_inventory[i].expire_time ){
			...
			// --- CAP DE PRECIO CUSTOM (Anti-Inflación) ---
			if (price > 2000 && (sd.inventory_data[i]->type == IT_WEAPON || sd.inventory_data[i]->type == IT_ARMOR)) {
				price = 2000;
			}
		}
...
/// Presents a list of items, that can be processed by Arrow/Ammo Crafting (ZC_MAKINGARROW_LIST).
void clif_arrow_create_list(map_session_data& sd, uint16 skill_id) {
	...
	// --- LA SOLUCIÓN: Usar un puntero a la clase base común ---
	TypesafeYamlDatabase<t_itemid, s_skill_arrow_db>* db;
	
	if (skill_id == GS_AMMOCRAFT) {
		db = &skill_bullet_db;
	} else {
		db = &skill_arrow_db;
	}
	...
	// Fíjate en el asterisco *db para "desempaquetar" el puntero
	for (const auto &it : *db) {
		t_itemid material_id = it.second->nameid;

		if (!item_db.exists(material_id)) continue;

		int32 index = pc_search_inventory(&sd, material_id);
		if (index < 0 || sd.inventory.u.items_inventory[index].equip || !sd.inventory.u.items_inventory[index].identify)
			continue;

		p->items[count].itemId = client_nameid(material_id);
		p->packetLength += static_cast<decltype(p->packetLength)>(sizeof(p->items[0]));
		count++;
	}
...
void clif_skill_damage( const block_list& src, const block_list& dst, t_tick tick, int32 sdelay, int32 ddelay, int64 sdamage, int16 div, uint16 skill_id, uint16 skill_lv, e_damage_type type ){
	...
	// --- INICIO DEL DISFRAZ DEL HACHA VOLADORA ---
	// Si el motor calcula daño de MC_TOMAHAWK, le decimos al cliente que dibuje ITM_TOMAHAWK
	if (skill_id == MC_TOMAHAWK) {
		skill_id = ITM_TOMAHAWK;
	}
	// ---------------------------------------------
...
void clif_parse_SelectArrow(int32 fd,map_session_data *sd) {
	...
	switch (sd->menuskill_id) {
		case AC_MAKINGARROW:
		case GS_AMMOCRAFT: // <--- AÑADIDO: Ambas habilidades usan la misma función de crafteo
