...
void mob_process_drop_list(std::shared_ptr<s_item_drop_list>& list, bool loot)
{
	...
	for (std::shared_ptr<s_item_drop>& ditem : list->items) {
		...
		// Extraemos la ID del ítem para comprobar sus propiedades
		struct item_data* option_id = itemdb_search(ditem->item_data.nameid);
		if (option_id != nullptr) {
			// Invocamos tu motor para que rellene los huecos ANTES de tocar el suelo
			pc_apply_random_option(option_id, ditem->item_data);
		}
...
int32 mob_dead(mob_data *md, block_list *src, int32 type)
{
	...
	{ // Item Drop
		...
		for( const std::shared_ptr<s_mob_drop>& entry : md->db->dropitem ){
			...
			// 1. Calculamos el rate base inicial (4 argumentos para tu versión)
			drop_rate = mob_getdroprate(src, md->db, entry->rate, drop_modifier);

			// 2. APLICAMOS LA DICTADURA FÍSICA (Sobrescribimos el resultado final)
			...
			// 6. Llamada final al drop corregida (6 argumentos)
			// Usamos entry->rate en lugar de custom_base_rate si autoloot_adjust está desactivado
