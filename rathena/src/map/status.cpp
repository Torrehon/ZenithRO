...
uint16 status_base_atk(const block_list *bl, const struct status_data *status, int32 level)
{
	...
	if (flag == 1) { // Si es rango, intercambia STR por DEX
#ifdef RENEWAL
		dstr =
#endif
		str = status->dex;
...
#else
			if (flag == 2) { // --- CUSTOM: Fórmula Híbrida Huuma (50% Melee / 50% Rango, Sin LUK) ---
				dstr = str / 10;
				int32 ddex = dex / 10; // Sacamos la decena de la DEX para su bono exponencial
				// Calculamos el daño total que daría cada uno por separado
				int32 total_str = str + (dstr * dstr);
				int32 total_dex = dex + (ddex * ddex);
				
				/* 
				   SINERGIA: Sumamos ambos y aplicamos un multiplicador.
				   Dividir por 15 y multiplicar por 10 equivale a un 66% de cada uno.
				   Total = 133% de potencial comparado con un arma normal.
				*/
...
#else
			// --- INICIO CÓDIGO CUSTOM: ANGELUS MAX HP PRE-RENEWAL ---
			if (sc->getSCE(SC_ANGELUS))
				bonus += sc->getSCE(SC_ANGELUS)->val1 * 100;
			// --- FIN CÓDIGO CUSTOM ---
#endif
...
bool status_calc_weight(map_session_data *sd, enum e_status_calc_weight_opt flag)
{
	...
	if (flag&CALCWT_ITEM) {
		...
		for(i = 0; i < MAX_INVENTORY; i++) {
			...
			// 1. Suma el peso del objeto base
			sd->weight += sd->inventory_data[i]->weight * sd->inventory.u.items_inventory[i].amount;

			// --- INICIO CUSTOM: Sumar el peso de las cartas insertadas ---
...
// ------ ATTACK CALCULATION ------
	...
	if (sd->status.weapon == W_2HSWORD) {
		...
		// Comprobamos el nivel de Two-Handed Sword Mastery
		if ((skill = pc_checkskill(sd, SM_TWOHAND)) > 0) {
			
			// Misma fórmula exacta: (Nivel Skill * (Job - 1)) / 14
			int32 jlv = (sd->status.job_level > 0) ? (sd->status.job_level - 1) : 0;
			base_status->batk += (skill * jlv) / 10;
		}
	}
	...
	if ((skill = pc_checkskill(sd, TK_HPTIME)) > 0) {		
		// Condición: Si Soul Channeling (Antigua TK_ENJOYREST) está aprendida, la skill se cancela.
		    if (pc_checkskill(sd, TK_SPTIME) == 0) {
			   int32 jlv = (sd->status.job_level > 0) ? (sd->status.job_level - 1) : 0;
			   base_status->batk += (skill * jlv) / 10;
		    }
	    }
	...
	if ((skill = pc_checkskill(sd, TK_SPTIME)) > 0) {
		if (pc_checkskill(sd, TK_HPTIME) == 0) { // Cancelado por Iron Body
			// 1. Obtenemos la INT total del personaje calculada hasta este punto
			int32 total_int = base_status->int_;
			
			// 2. Recreamos la fórmula de MATK base de Pre-Renewal
			int32 m_min = total_int + (total_int / 7) * (total_int / 7);
			int32 m_max = total_int + (total_int / 5) * (total_int / 5);
			
			// 3. Calculamos el bono plano que da la propia pasiva
			int32 jlv = (sd->status.job_level > 0) ? (sd->status.job_level - 1) : 0;
			int32 bonus_plano = (skill * jlv) / 10;
			
			// 4. Inyectamos todo al BATK:
			// Sumamos el min, el max y el bono (x2, ya que afecta a min y max).
			// Al dividir el total entre 4, obtenemos exactamente el 50% de la media.
			base_status->batk += (m_min + m_max + (bonus_plano * 2)) / 4;
		}
	}
	// --- FIN: Soul Channeling ---
...
// ----- MISC CALCULATION -----
    ...
    // --- INICIO: Iron Body Custom (Vit Def) ---
    if ((skill = pc_checkskill(sd, TK_HPTIME)) > 0) {
			// Condición: Si Soul Channeling está aprendida, la skill se cancela.
			if (pc_checkskill(sd, TK_SPTIME) == 0) {
				int32 jlv = (sd->status.job_level > 0) ? (sd->status.job_level - 1) : 0;
				base_status->def2 += (skill * 2) + ((skill * jlv) / 20);
			}
	}
	// --- FIN: Iron Body Custom (Vit Def) ---
	if(sd->def2_rate < 0)
		sd->def2_rate = 0;
	if(sd->def2_rate != 100)
...
// ----- HIT CALCULATION -----
	...
	if ((skill = pc_checkskill(sd, TF_MISS)) > 0) {
		// 1. La mecánica original (+4 para 2nd classes, +3 para el resto)
		base_status->flee += skill * (sd->class_&JOBL_2 && (sd->class_&MAPID_FIRSTMASK) == MAPID_THIEF ? 4 : 3);
		
		// 2. Tu nueva mecánica (+2 Flee por cada 10 niveles de Job)
		int jlvl_bonus = (sd->status.job_level / 10) * 2;
...
// ----- CRITICAL CALCULATION -----
	...
	if ((skill = pc_checkskill(sd, PR_MACEMASTERY)) > 0 && (sd->status.weapon == W_MACE || sd->status.weapon == W_2HMACE))
		base_status->cri += skill * 10;
#endif
// --- INICIO: 2-Hand Sword Mastery Custom CRIT ---
	// La fórmula aplica solo si tienes la skill aprendida (>0), sin importar a qué nivel esté.
	if (sd->status.weapon == W_2HSWORD) {
		if (pc_checkskill(sd, SM_TWOHAND) > 0) {
			
			int jlvl = sd->status.job_level;
			

			// Usamos 30 en lugar de 3 para aplicar el multiplicador x10 de rAthena automáticamente.

			base_status->cri += (jlvl * 30) / 10;
		}
	}
	// --- FIN: 2-Hand Sword Mastery Custom CRIT ---
...
// ----- ASPD CALCULATION -----
	...
	if (sd->status.weapon == W_HUUMA || sd->status.weapon == W_DAGGER) {
		if ((skill = pc_checkskill(sd, NJ_TOBIDOUGU)) > 0) {
			...
			// Aplicamos tu fórmula: (sklv * 5) + (jlvl * 5 / 7)
			int aspd_bonus = (skill * 5) + ((jlvl * 5) / 10);
...
// ----- MISC CALCULATIONS -----
	...
	// --- INICIO: Bugei (Daño a Distancia Pasivo) ---
	if ((skill = pc_checkskill(sd, NJ_TOBIDOUGU)) > 0) {
		sd->bonus.long_attack_atk_rate += (sd->status.job_level) / 10;
	}
	// --- FIN: Bugei (Daño a Distancia Pasivo) ---
	// --- INICIO: Firearms Mastery (Daño a Distancia Pasivo) ---
	if (sd->weapontype1 == W_RIFLE || sd->weapontype1 == W_SHOTGUN || sd->weapontype1 == W_GRENADE) {
		if (pc_checkskill(sd, GS_SINGLEACTION) > 0) {
			sd->bonus.long_attack_atk_rate += (sd->status.job_level / 10);
		}
	}
	// --- FIN: Firearms Mastery (Daño a Distancia Pasivo) ---
	// --- INICIO: Ninpo (Daño Mágico Elemental Pasivo) ---
	if ((skill = pc_checkskill(sd, NJ_NINPOU)) > 0) {
		sd->indexed_bonus.magic_atk_ele[ELE_FIRE] += skill;
		sd->indexed_bonus.magic_atk_ele[ELE_WATER] += skill;
		sd->indexed_bonus.magic_atk_ele[ELE_EARTH] += skill;
	}
	// --- FIN: Ninpo (Daño Mágico Elemental Pasivo) ---

	if(!sc->empty()) {
		if(sc->getSCE(SC_CONCENTRATE)) { // Update the card-bonus data
			sc->getSCE(SC_CONCENTRATE)->val3 = sd->indexed_bonus.param_bonus[1]; // Agi
			
			// Actualizamos val4 con DEX e INT empaquetadas
			sc->getSCE(SC_CONCENTRATE)->val4 = (sd->indexed_bonus.param_bonus[4] & 0xFFFF) | ((sd->indexed_bonus.param_bonus[3] & 0xFFFF) << 16);
		}
		...
		// --- INICIO CUSTOM: DUELIST Daño Crítico ---
		if (sc->getSCE(SC_DUELIST)) {
...
void status_calc_regen(block_list *bl, struct status_data *status, struct regen_data *regen)
{
	...
	val = (status->vit/4) + max(1, status->max_hp/200)* 4;
	if (status->vit >= 90) {
		val += 5;
	}
	...
	val = 1 + (status->int_/4) + (status->max_sp/100);
	if( status->int_ >= 100 )
		val += ((status->int_-100) / 2) + 4;
	...
	if( sd ) {
		...
		if( (skill=pc_checkskill(sd,SM_RECOVERY)) > 0 )
			val += skill*2 + skill*2*status->max_hp/500;

		sregen->hp = cap_value(val, 0, SHRT_MAX);

		val = 0;
		/*if( (skill=pc_checkskill(sd,MG_SRECOVERY)) > 0 ) //SP Recovery anulado
			val += skill*3 + skill*status->max_sp/500;*/
		/*if( (skill=pc_checkskill(sd,NJ_NINPOU)) > 0 )
			val += skill*3 + skill*status->max_sp/500;*/   // Ninpou anulado
			...
			switch( type ){
				...
				case SC_HIDING:
					if (bl.type != BL_PC) {
						...
						map_session_data* sd_ptr = (map_session_data*)&bl;
						// Verificamos si NO tiene Tunnel Drive Y NO tiene Shadow Path
						if (pc_checkskill(sd_ptr, RG_TUNNELDRIVE) <= 0 && pc_checkskill(sd_ptr, NJ_SHADOWPATH) <= 0) {
							restriction = true;
						}
					}
					break;
	...
	**/
	...
	if(flag[SCB_MATK]) {
		...
		if (sd != nullptr) {
			...
			// --- INICIO CÓDIGO CUSTOM: MATK POR REFINE (AMBAS MANOS) ---
			int hands[2] = { EQI_HAND_R, EQI_HAND_L };
			for (int i = 0; i < 2; i++) {
				int eq_index = sd->equip_index[hands[i]];
				
				// Verificamos que tenga algo equipado y que sea un ARMA
				if (eq_index >= 0 && sd->inventory_data[eq_index] != nullptr && sd->inventory_data[eq_index]->type == IT_WEAPON) {
					int refine_lvl = sd->inventory.u.items_inventory[eq_index].refine;
					
					if (refine_lvl > 0) {
						int wlv = sd->inventory_data[eq_index]->weapon_level;
						int refine_matk = 0;

						// Escala de incrementos por nivel de arma
						switch (wlv) {
							case 1: refine_matk = refine_lvl * 1; break;
							case 2: refine_matk = refine_lvl * 2; break;
							case 3: refine_matk = refine_lvl * 4; break;
							case 4: refine_matk = refine_lvl * 5; break;
							default: refine_matk = refine_lvl * 1; break;
						}

						// Sumamos el bono al MATK temporal
						matk_min += refine_matk;
						matk_max += refine_matk;
					}
				}
			}
			// --- FIN CÓDIGO CUSTOM ---
...
static uint16 status_calc_int(block_list *bl, status_change *sc, int32 int_)
{
	...
	// --- AÑADIDO: Bono de INT por Attention Concentrate ---
	if(sc->getSCE(SC_CONCENTRATE) && !sc->getSCE(SC_QUAGMIRE))
		int_ += (int_ - ((sc->getSCE(SC_CONCENTRATE)->val4 >> 16) & 0xFFFF)) * sc->getSCE(SC_CONCENTRATE)->val2 / 100;
	if(sc->getSCE(SC_INCALLSTATUS))
...
static uint16 status_calc_dex(block_list *bl, status_change *sc, int32 dex)
{
	...
	if(sc->getSCE(SC_CONCENTRATE) && !sc->getSCE(SC_QUAGMIRE))
		dex += (dex - (sc->getSCE(SC_CONCENTRATE)->val4 & 0xFFFF)) * sc->getSCE(SC_CONCENTRATE)->val2 / 100;
	if(sc->getSCE(SC_INCALLSTATUS))
...
static int16 status_calc_critical(block_list *bl, status_change *sc, int32 critical)
{
	...
	if (sc->getSCE(SC_TWOHANDQUICKEN)) {
		// Usamos 'bl' directamente, que es el parámetro de la función en la que estás
		map_session_data* sd = BL_CAST(BL_PC, bl); 
		
		if (sd != nullptr && pc_checkskill(sd, KN_BLADERSOUL) > 0) {
			critical += sc->getSCE(SC_TWOHANDQUICKEN)->val1 * 10;
		}
	}
#ifdef RENEWAL
...
static int16 status_calc_hit(block_list *bl, status_change *sc, int32 hit)
{
	...
	if (sc->getSCE(SC_TWOHANDQUICKEN)) {
		// Lo mismo aquí, invocamos a 'bl' a secas
		map_session_data* sd = BL_CAST(BL_PC, bl);
...
static int16 status_calc_def2(block_list *bl, status_change *sc, int32 def2)
{
	...
	// Abrimos llaves para poder meter múltiples líneas de código de forma segura
	if(sc->getSCE(SC_ANGELUS)) {
#ifdef RENEWAL /// The VIT stat bonus is boosted by angelus [RENEWAL]
		def2 += status_get_vit(bl) / 2 * sc->getSCE(SC_ANGELUS)->val2/100;
#else
		// --- INICIO CÓDIGO CUSTOM: ANGELUS DEF PRE-RENEWAL ---
		int skill_lv = sc->getSCE(SC_ANGELUS)->val1;
		int base_level = status_get_lv(bl); 
		
		int flat_bonus = (base_level + 1) / 2;
		int percentage_multiplier = 6 * skill_lv; // Ej: 30% al nivel 5
		
		// 1. Primero añadimos el bono plano por nivel a tu Vit Def actual
		def2 += flat_bonus;

		// 2. Ahora calculamos el porcentaje sobre el NUEVO TOTAL (VIT + Nivel)
		int percentage_bonus = (def2 * percentage_multiplier) / 100;

		// 3. Sumamos ese porcentaje extra para obtener el resultado definitivo
...
static int16 status_calc_aspd_rate(block_list *bl, status_change *sc, int32 aspd_rate)
{
	...
	if (sc->getSCE(SC_INCREASEAGI))
		aspd_rate -= 5 * sc->getSCE(SC_INCREASEAGI)->val1;
    if (sc->getSCE(SC_DUELIST))
		aspd_rate -= 100;
	if (sc->getSCE(SC_TWOHANDQUICKEN)) {
		map_session_data* sd = BL_CAST(BL_PC, bl);
		// Tomamos el valor que rAthena da por defecto al lanzar la skill (30)
		int quicken_mod = sc->getSCE(SC_TWOHANDQUICKEN)->val2; 
		
		if (sd != nullptr) {
			if (sd->weapontype1 == W_1HSWORD) {
				quicken_mod = 200; // Forzamos 20% si es espada de 1 mano
			} else if (sd->weapontype1 == W_2HSWORD) {
				quicken_mod = 300; // Forzamos 30% si es de 2 manos
			} else {
				quicken_mod = 0;  // Anula el bono de velocidad si usa otra cosa
			}
		}
	...
	// --- Inicio Custom Skill: TF_LIGHTFINGERS ---
	if (sc->getSCE(SC_LIGHTFINGERS)) {
		// Escala 1000 = 100%. Queremos un 2% por nivel.
		// Nivel 5 * 20 = 100 (10% de bono real de ASPD)
...
static bool status_change_start_post_delay(block_list* src, block_list* bl, sc_type type, int32 val1, int32 val2, int32 val3, int32 val4, int32 tick, uint8 flag)
{
	...
	switch (type) {
		...
		case SC_SIGNUMCRUCIS:
			// Solo evita afectar a jugadores. ¡Todos los monstruos recibirán el debuff!
			if (bl->type == BL_PC)
				return false;
			break;
		case SC_EXPOSED:
		...
		case SC_GUARD_STANCE:
			if (sd) {
				// Solo necesitamos forzar el recalcular los stats
				status_calc_pc(sd, SCO_NONE);
			}
			break; // Muy importante: usa break para que el servidor siga con su lógica normal
		case SC_CLOAKING:
	...
	switch(type)
	{
		...
		case SC_CONCENTRATE:
			...
			if (sd) { // Store the card-bonus data that should not count in the %
				...
				val4 = (sd->indexed_bonus.param_bonus[4] & 0xFFFF) | ((sd->indexed_bonus.param_bonus[3] & 0xFFFF) << 16);
			} else
				val3 = val4 = 0;
			break;
...
int32 status_change_end( block_list* bl, enum sc_type type, int32 tid ){
	...
	switch(type) {
		...
		case SC_RUN:
		{
			...
			if (begin_spurt && val1 >= 3 &&
				DIFF_TICK(gettick(), starttick) <= 1000 &&
				(!sd || ((sd->weapontype1 == W_FIST || sd->weapontype1 == W_BANDAGE) && sd->weapontype2 == W_FIST))
			)
		...
		case SC_GUARD_STANCE:
			if (sd) {
				...
				// Los 7 argumentos: bl, icono, bandera (0=off), tick, val1, val2, val3
				clif_status_change(bl, EFST_GUARD_STANCE, 1, sec, 0, 0, 0);
...
static int32 status_natural_heal(block_list* bl, va_list args)
{
	...
	if (flag&RGN_HP) {
		...
		if (regen->tick.hp + rate <= natural_heal_prev_tick) {
			...
			if (status->hp >= status->max_hp)
				...
				int32 heal_amount_hp = (sd && sd->state.rest) ? (regen->hp * 5) : regen->hp;
				if (status_heal(bl, heal_amount_hp, 0, 1) < heal_amount_hp)
					flag &= ~RGN_SHP;
			} // <--- CIERRA LLAVE A
		}
	} else {
	...
	if(flag&RGN_SP) {
		...
		if (regen->tick.sp + rate <= natural_heal_prev_tick) {
			...
			if (status->sp >= status->max_sp)
				flag &= ~(RGN_SP | RGN_SSP);
			else { // <--- LLAVE B (MUY IMPORTANTE)
				int32 heal_amount_sp = (sd && sd->state.rest) ? (regen->sp * 5) : regen->sp;
