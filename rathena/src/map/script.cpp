...
BUILDIN_FUNC(getitem)
{
	...
	for (i = 0; i < amount; i += get_count)
	{
		...
		{
			// --- INICIO CUSTOM RANDOM OPTIONS (QUESTS) ---
			if (id != nullptr) {
				// Limpiamos los slots de opciones por si el NPC da varios ítems seguidos,
				// para garantizar que tu escudo anti-reroll permita crear opciones únicas para cada uno.
				memset(it.option, 0, sizeof(it.option));
				pc_apply_random_option(id.get(), it);
			}
			// --- FIN CUSTOM RANDOM OPTIONS (QUESTS) ---
...
BUILDIN_FUNC(getitem2)
{
	...
	if( item_data ) {
		...
		for (int32 i = 0; i < amount; i += get_count)
		{
			...
			if (!pet_create_egg(sd, nameid))
			{
				// --- INICIO CUSTOM RANDOM OPTIONS (QUESTS/EVENTOS) ---
				// offset == 0 asegura que no sobreescribimos opciones si se usa getitem3/getitem4
				if (item_data != nullptr && offset == 0) {
					memset(item_tmp.option, 0, sizeof(item_tmp.option));
					pc_apply_random_option(item_data.get(), item_tmp);
				}
				// --- FIN CUSTOM RANDOM OPTIONS (QUESTS/EVENTOS) ---
...
/*==========================================
 * Comando Custom: setitemoption <inv_idx>, <opt_slot>, <opt_id>, <opt_val>;
 *==========================================*/
BUILDIN_FUNC(setitemoption)
{
	TBL_PC *sd = NULL;
	
	// Llamamos a la función original directamente para evitar el error de conversión
	if (!script_rid2sd_(st, &sd, "setitemoption") || sd == NULL) 
		return SCRIPT_CMD_FAILURE;

	int idx = script_getnum(st, 2); // Índice real del inventario
	int opt_idx = script_getnum(st, 3); // Slot de la opción (0 a 4)
	int opt_id = script_getnum(st, 4); // ID de la opción
	int opt_val = script_getnum(st, 5); // Nuevo valor

	if (idx < 0 || idx >= MAX_INVENTORY) return SCRIPT_CMD_FAILURE;
	if (opt_idx < 0 || opt_idx >= MAX_ITEM_RDM_OPT) return SCRIPT_CMD_FAILURE;
...
struct script_function buildin_func[] = {
	...
	BUILDIN_DEF(setitemoption, "iiii"),
