...
void SkillPangVoice::modifyDamageData(Damage& dmg, const block_list& src, const block_list& target, uint16 skill_lv) const {
...
void SkillPangVoice::castendDamageId(block_list *src, block_list *target, uint16 skill_lv, t_tick tick, int32& flag) const {
	// 1. Lanzamos el ataque mágico (esto llamará automáticamente a calculateSkillRatio y a modifyDamageData)
	...
	// 3. Consumo de la marca Unnerved (lo hacemos al final del todo)
	const map_session_data* sd = BL_CAST(BL_PC, src);
