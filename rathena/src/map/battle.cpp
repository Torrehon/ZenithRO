...
int64 battle_addmastery(map_session_data *sd,block_list *target,int64 dmg,int32 type)
{
	...
	if((skill = pc_checkskill(sd,AL_DEMONBANE)) > 0 &&
		...
		damage += static_cast<decltype(damage)>((10 * skill) + ((sd->status.base_level + 1) / 5));
...
void battle_consume_ammo(map_session_data*sd, int32 skill, int32 lv)
{
	...
	if (skill) {
		...
		// --- CONSUMO DINÁMICO DE RAPID SHOWER ---
...
static int32 battle_range_type(const block_list* src, const block_list* target, uint16 skill_id, uint16 skill_lv)
{
	...
	switch (skill_id) {
		...
		case SS_HITOUAKUMU:
			return BF_LONG;
		case NJ_KASUMIKIRI: // Ahora Shadow Dash es la que salta, pero su daño es melee
...
static bool is_attack_critical(struct Damage* wd, block_list *src, const block_list *target, int32 skill_id, int32 skill_lv, bool first_call)
{
	...
	if( sstatus->cri )
	{
		...
		//if(wd->type == DMG_MULTI_HIT){	//Multiple Hit Attack Skills.
			...
			// if(pc_checkskill(sd,TF_DOUBLE) && !skill_get_nk(TF_DOUBLE,NK_CRITICAL)) //Double Attack //Double attack ahora SI HACE CRIT
				// return false;
		//}
		...
		switch(skill_id) {
			...
			case AC_HEAVYSHOT:
				...
				break;
			// --- FIN CUSTOM ---
			// --- INICIO CUSTOM: Fatal Blow ---
			case SM_BASH:
				// Solo si tiene Fatal Blow y el Bash es nivel 10
				if (sd && pc_checkskill(sd, SM_FATALBLOW) > 0 && skill_lv == 10) {
					cri += 500; // +50% de probabilidad
				} else {
					// Si no tiene la quest o no es lv 10, abortamos el crítico
					return false; 
				}
				break;
            // --- FIN CUSTOM --- 
	        // --- INICIO CUSTOM: Shattering Kicks (Counter Kick) ---
			case TK_COUNTER:
				...
				break;
	        // --- FIN CUSTOM ---
			// --- INICIO CUSTOM: Pierce (Lancer Soul) ---
			case KN_PIERCE:
				// sd ya está declarado más arriba en esta función, así que podemos usarlo directo
				if (sd != nullptr && pc_checkskill(sd, KN_LANCERSOUL) > 0) {
					cri += 500; // +50 de Critical
				}
				break;
			// --- FIN CUSTOM ---
#ifdef RENEWAL
...
int32 battle_get_weapon_element(const Damage& wd, const block_list& src, const block_list& target, uint16 skill_id, uint16 skill_lv, equip_index weapon_position, bool calc_for_damage_only) {
	...
	case TF_SPRINKLESAND:
		element = ELE_NEUTRAL; // Por defecto siempre será Neutral (ignora el arma)
		if (sd != nullptr && sd->equip_index[EQI_SHADOW_SHOES] >= 0) {
			int index = sd->equip_index[EQI_SHADOW_SHOES];
			// Comprobamos que la información del ítem existe en ese slot
			if (sd->inventory_data[index] != nullptr) {
				uint32 nameid = sd->inventory_data[index]->nameid;

				if (nameid == 60990) {
					element = ELE_FIRE;   // Red Sand Bag
				}
				else if (nameid == 60991) {
					element = ELE_WATER;  // Blue Sand Bag
				}
				else if (nameid == 60992) {
					element = ELE_WIND;   // Yellow Sand Bag
				}
				else if (nameid == 60993) {
					element = ELE_EARTH;  // Green Sand Bag
				}
			}
		}
		break;
			...
			if(sd) {
                ...
                if(sd->status.party_id && (skill=pc_checkskill(sd,TK_POWER)) > 0) {
					if( (i = party_foreachsamemap(party_sub_count, sd, 0)) > 1 ) { // exclude the player himself [Inkfish]
						...
						i -= 1; 
						ATK_ADDRATE(wd->damage, wd->damage2, 4*skill*i);
					}
				}
#else
...
static void battle_calc_multi_attack(struct Damage* wd, block_list *src,block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	if( sd && !skill_id ) {	// if no skill_id passed, check for double attack [helvetica]
		...
		// Chain Action escalable: 7% por nivel (Revólver) y 3% por nivel (Gatling)
		if( wd->div_ == 1 ) {
			int chain_rate = 0; // Variable para guardar el chance
			
			if (sd->weapontype1 == W_REVOLVER && (skill_lv = pc_checkskill(sd, GS_CHAINACTION)) > 0) {
				chain_rate = 7 * skill_lv;
			} 
			else if (sd->weapontype1 == W_GATLING && (skill_lv = pc_checkskill(sd, GS_CHAINACTION)) > 0) {
				chain_rate = 3 * skill_lv;
			} 
			else if (sc != nullptr && !sc->empty() && sc->getSCE(SC_E_CHAIN) && (skill_lv = sc->getSCE(SC_E_CHAIN)->val1) > 0) {
				chain_rate = 5 * skill_lv; // Mantenemos el 5% original para el buff Eternal Chain por seguridad
			}

			// Si hay chance acumulado y el RNG es menor que ese chance, se activa
			if (chain_rate > 0 && rnd() % 100 < chain_rate) {
...
static void battle_calc_defense_reduction( Damage* wd, block_list* src, block_list* target, uint16 skill_id, uint16 skill_lv )
{
	...
	if (tsd) {	//Sd vit-eq
		...
		if (src->type == BL_MOB && (battle_check_undead(sstatus->race, sstatus->def_ele) || sstatus->race == RC_DEMON) && //This bonus already doesn't work vs players
			...
			vit_def += (6 * skill) + ((tsd->status.base_level + 1) / 10);
	...
	if (sd) {
		...
		if(sd->status.weapon == W_KATAR && !skill_id) { //Katars (offhand damage only applies to normal attacks, tested on Aegis 10.2)
			...
			if (wd->damage2) {
				...
				else if(sd->class_ == JOB_NINJA || sd->class_ == MAPID_NINJA) {
...
static struct Damage battle_calc_weapon_attack(block_list *src, block_list *target, uint16 skill_id, uint16 skill_lv, int32 wflag)
{
	...
	if (tsc != nullptr && tsc->getSCE(SC_EXPOSED) && (wd.flag & BF_LONG)) {
...
//Apply the physical part of the skill's damage. [Skotlex]
			...
			case AL_CRUCIS:
		{
			...
			if (sstatus->matk_max > sstatus->matk_min) {
				base_matk += rnd() % (sstatus->matk_max - sstatus->matk_min);
			}

			int64 total_atk = (int64)sstatus->batk + sstatus->rhw.atk + sstatus->lhw.atk;
			
			// Sumamos tu daño base primero (ATK + MATK)
			int64 base_dmg = total_atk + base_matk;

			bool is_undead_demon = (tstatus->race == RC_DEMON || battle_check_undead(tstatus->race, tstatus->def_ele));

			int64 ratio; // Este será nuestro multiplicador en porcentaje

			if (is_undead_demon) {
				
				ratio = 50 + (20 * skill_lv);
			} else {
				
				ratio = 100 + (20 * skill_lv);
			}

			// Aplicamos el ratio dinámico (Multiplicamos por el porcentaje y dividimos entre 100)
			ad.damage = (base_dmg * ratio) / 100;
		...
		case TK_SKICK:
		{
			...
			// 1. Obtenemos el MATK base (fluctuación entre mínimo y máximo)
			int32 base_matk = sstatus->matk_min;
			if (sstatus->matk_max > sstatus->matk_min) {
				// Nota: Le añado un "+ 1" respecto a tu ejemplo.
				// rnd() % X devuelve de 0 a X-1. Con el +1 aseguramos que el daño máximo pueda salir.
				base_matk += rnd() % (sstatus->matk_max - sstatus->matk_min + 1);
			}

			// 2. Obtenemos el ATK total (Status + Arma derecha + Arma izquierda)
			int64 total_atk = (int64)sstatus->batk + sstatus->rhw.atk + sstatus->lhw.atk;

			// 3. Fórmula: 100% ATK + 250% MATK
			// El ATK se suma directo (100%), el MATK lo escalamos al 250%
			int64 matk_portion = (base_matk * 250) / 100;

			// 4. Lo inyectamos en el daño base del ataque mágico
	...
	// --- INICIO CUSTOM: Magias Ninja (Crítico Visual y Modo Chunin) ---
	if (skill_id == NJ_KOUENKA || skill_id == NJ_HYOUSENSOU || skill_id == NJ_HUUJIN) {
		
		if (sd && sd->weapontype1 == W_DAGGER && sd->weapontype2 == W_DAGGER) {
			ad.damage = ad.damage * 80 / 100;
			ad.div_ *= 2; 
		}

		// 2. Crítico Mágico (Independiente del arma, basado en el CRI del usuario)
        if (sd) {
	...
	if (sd && sd->status.party_id > 0 && (i = pc_checkskill(sd, TK_POWER)) > 0) {
		int party_members = party_foreachsamemap(party_sub_count, sd, 0);
		if (party_members > 1) {
			party_members -= 1; // Excluye al propio jugador
			
			// Añadimos el 2% al daño mágico por compañero
			MATK_ADDRATE(2 * i * party_members);
		}
	}
	// --- FIN: Custom Taekwon: Fighting Chant (Kihop) MATK (3%) ---
	//Apply DAMAGE_DIV_FIX and check for min damage
...
if (tsc && tsc->getSCE(SC_AUTOCOUNTER) && status_check_skilluse(target, src, KN_AUTOCOUNTER, 1)) {
		// Ya no calculamos hacia dónde miran los personajes, solo la distancia
		int32 dist = distance_bl(src, target);

		// Si el enemigo está a rango de ataque (ignorando la dirección), bloqueamos el golpe
