...
uint8 npc_selllist(map_session_data* sd, int32 list_length, const PACKET_CZ_PC_SELL_ITEMLIST_sub* item_list)
{
	...
	for( i = 0; i < list_length; i++ )
	{
		...
		if (battle_config.rental_item_novalue && sd->inventory.u.items_inventory[idx].expire_time) {
			...
			int32 base_price = sd->inventory_data[idx]->value_sell;
			if (base_price > 2000 && (sd->inventory_data[idx]->type == IT_WEAPON || sd->inventory_data[idx]->type == IT_ARMOR)) {
				base_price = 2000;
			}
			value = pc_modifysellvalue(sd, base_price);
			// ---------------------------------------------
		}
