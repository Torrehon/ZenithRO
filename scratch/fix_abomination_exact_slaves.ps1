...
$filePreReDb = "d:\SERVER_RO\LevitationRO\rathena\db\pre-re\mob_db.yml"
$filePreReSkill = "d:\SERVER_RO\LevitationRO\rathena\db\pre-re\mob_skill_db.txt"

# 1. Update mob_db.yml in pre-re
$dbContent = Get-Content -Path $filePreReDb -Raw
$slavesYaml = @"
    Slaves:
      - Mob: ZOMBIE_GUARD
        Count: 3
...
if ($dbContent.Contains("AegisName: AS_D_RAGGED_GOLEM")) {
    $dbContent = $dbContent.Replace("    Drops:", $slavesYaml)
    [System.IO.File]::WriteAllText($filePreReDb, $dbContent, $utf8NoBom)
    Write-Host "Added Slaves block to mob_db.yml for 3484!"
}

# 2. Update mob_skill_db.txt in pre-re
$skillLines = @(
    "// Abomination of Souls (Mob 3484 - Custom Reworked Kit with Verified Slaves: 3x Zombie Guard, 1x Fallen Crusader, 1x Lost Knight)",
...
Write-Host "Updated db/pre-re/mob_skill_db.txt with exact slaves!"
