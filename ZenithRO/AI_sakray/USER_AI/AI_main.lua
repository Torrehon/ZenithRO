...
function doInit(myid)
	...
	if IsHomun(myid) == 1 then
		...
		local base_type = htype
		if htype < 17 then
			base_type = modulo(htype, 4)
		else
			if htype == EIRA then
				base_type = 1 -- Lif
			elseif htype == BAYERI or htype == DIETER then
				base_type = 2 -- Amistr
			elseif htype == SERA then
				base_type = 0 -- Vanilmirth
			elseif htype == ELEANOR then
				base_type = 3 -- Filir
			end
		end

		if base_type == 1 then -- Lif / Eira: Passive Support (only defend or heal)
			AutoMobMode = 3
			SuperPassive = 0
		elseif base_type == 2 then -- Amistr / Bayeri / Dieter: Defensive Tank (defend owner)
			AutoMobMode = 2
			SuperPassive = 0
		elseif base_type == 3 then -- Filir / Eleanor: Highly Aggressive DPS
			AutoMobMode = 0
			SuperPassive = 0
		elseif base_type == 0 then -- Vanilmirth / Sera: Aggressive Magic DPS
			AutoMobMode = 0
			SuperPassive = 0
		end
	end
...
function DoAutoBuffs(buffmode)
	...
	if (UseOffensiveBuff == buffmode and QuickenTimeout ~=-1) then
		if (GetTick() > QuickenTimeout) then
			...
			elseif level==0 then
				...
				local dur = GetSkillInfo(skill,8,level)
				if dur == nil or dur < 10000 then dur = 30000 end
				QuickenTimeout = GetTick() + dur
				if (GetSkillInfo(skill,3,level) <= GetV(V_SP,MyID)) then
					DoSkill(skill,level,MyID,2)
					AutoSkillTimeout = GetTick() + 2000
					UpdateTimeoutFile()
					return
				end
			end
	...
	if (UseDefensiveBuff == buffmode and GuardTimeout ~=-1) then
		if (GetTick() > GuardTimeout) then
			...
			elseif level==0 then
				-- skill in cooldown
			elseif (skill==HAMI_BULWARK or skill==HAMI_DEFENCE) and UseSmartBulwark ==1  then
				local dur = GetSkillInfo(skill,8,level)
				if dur == nil or dur < 10000 then dur = 30000 end
				GuardTimeout = GetTick() + dur
...
function FailSkillUse(mode) 
	...
	if SkillFailCount[mode] < SkillRetryLimit[mode] then
		...
		elseif mode==1 then
			GuardTimeout=GetTick()+15000
		elseif mode==2 then
			QuickenTimeout=GetTick()+15000
		elseif mode==3 then
			MagTimeout=1
		elseif mode==4 then
			SOffensiveTimeout=1
...
function AI(myid)
	...
	if (modulo(GetV(V_HOMUNTYPE,MyID),4)==AMISTR or OldHomunType==AMISTR) and UseCastleDefend ==1 then
		if GetTick() > AutoSkillTimeout then
