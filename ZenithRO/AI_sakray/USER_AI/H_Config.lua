AggroHP                  = 0	-- Ataca aunque este bajo de HP
AggroSP                  = 0
OldHomunType             = 2
UseSkillOnly             = -1 
UseAttackSkill           = 1
OpportunisticTargeting   = 0
DoNotChase               = 0
UseDanceAttack           = 1
SuperPassive             = 0
UseIdleWalk              = 0
IdleWalkSP               = 0
UseCastleRoute           = 0
RelativeRoute            = 1
UseCastleDefend          = 1
CastleDefendThreshold    = 2
IdleWalkDistance         = 4
FleeHP                   = 0
RescueOwnerLowHP         = 75
LiveMobID				 = 0

AttackSkillReserveSP     = 0 
AutoMobMode              = 0 	-- 0 = Agresivo: ataca cualquier enemigo a la vista
AutoMobCount             = 2
AutoComboMode            = 1
AutoComboSkill		=0
AutoComboSpheres         = 10
UseHomunSSkillChase      = 1
UseHomunSSkillAttack     = 1 
AoEMaximizeTargets       = 0
CastTimeRatio		= .80
UseEiraSilentBreeze      = 1
EiraSilentBreezeLevel    = 5 
UseBayeriStahlHorn       = 1
BayeriStahlHornLevel     = 10
UseBayeriHailegeStar     = 1
BayeriHailegeStarLevel   = 10
UseSeraParalyze          = 1
SeraParalyzeLevel        = 10
UseSeraPoisonMist        = 1 
SeraPoisonMistLevel      = 5
UseEleanorSonicClaw      = 1
EleanorSonicClawLevel    = 5
EleanorSilverveinLevel   = 10
EleanorMidnightLevel     = 10
UseDieterLavaSlide       = 1 
UseDieterVolcanicAsh	 = 1
UseDieterBlastForge      = 1
DieterLavaSlideLevel     = 10
UseDefensiveBuff         = 2	-- 2 = Solo en combate (nunca en reposo ni caminando)
UseOffensiveBuff         = 2	-- 2 = Solo en combate
UseProvokeOwner          = 0
ProvokeOwnerMobbed       = 3
LifEscapeLevel           = 5
FilirFlitLevel           = 1
FilirAccelLevel          = 1
AmiBulwarkLevel          = 5
UseSmartBulwark          = 0
UseBayeriAngriffModus    = 1
UseBayeriGoldenPherze    = 1
UseDieterMagmaFlow       = 1
UseDieterGraniticArmor   = 1
UseDieterPyroclastic     = 1
UseDieterTempering       = 1
UseEiraOveredBoost       = 1
HealSelfHP               = 65 
HealOwnerHP              = 70 
UseAutoHeal              = 1 
FollowStayBack           = 2
StationaryAggroDist      = 14	-- Radio de deteccion de enemigos en reposo
MobileAggroDist          = 12	-- Radio de deteccion de enemigos en movimiento
StationaryMoveBounds     = 16	-- Distancia maxima del owner que puede alejarse en reposo
MobileMoveBounds         = 14	-- Distancia maxima del owner que puede alejarse en movimiento
DoNotUseRest             = 0
RestXOff                 = 2
RestYOff                 = 0	
MoveSticky               = 0
MoveStickyFight          = 0
KiteMonsters             = 0
KiteBounds               = 10 
KiteStep                 = 5
KiteParanoidStep         = 2
KiteThreshold            = 3
KiteParanoidThreshold    = 2
KiteParanoid             = 0 
DefendStandby            = 0 
StickyStandby            = 1 
SpawnDelay               = 1000
AutoSkillDelay           = 2500	-- 2.5 segundos de cooldown minimo entre skills en combate
StandbyFriending         = 1  
MirAIFriending           = 1  
UseAvoid                 = 0  
TankMonsterLimit         = 4
LavaSlideMode            = 1
PoisonMistMode           = 1
AoEReserveSP             = 1

ChaseSPPause             = 0
ChaseSPPauseSP 			= -60
ChaseSPPauseTime         = 3000
AttackTimeLimit          = 0

UseBayeriSteinWand       = 1
BayeriSteinWandLevel     = 5
UseSteinWandSelfMob      = 2
UseSteinWandOwnerMob     = 2
UseSteinWandTele         = 0 
StienWandTelePause       = 3000
AssumeHomun              = 1
AttackLastFullSP         = 0
DanceMinSP               = 0
AutoSkillLimit           = 100
UseAutoPushback          = 0
AutoPushbackThreshold    = 2
AoEFixedLevel            = 0
UseSeraCallLegion        = 1
SeraCallLegionLevel      = 5
ForceKite                = 0
UseBerserkMobbed         = 0
UseBerserkSkill          = 0
UseBerserkAttack         = 0
Berserk_SkillAlways      = 0
Berserk_Dance            = 0
Berserk_IgnoreMinSP      = 0
Berserk_ComboAlways      = 0
PVPmode                  = 1
UseSeraPainkiller        = 3
DefensiveBuffOwnerMobbed = 0
PainkillerFriends        = 1
PainkillerFriendsSave    = 1
EleanorDoNotSwitchMode   = 0
LastSavedDate            = "Default Configuration 1.552"
UseEiraXenoSlasher       = 1
EiraXenoSlasherLevel     = 10
UseEiraEraseCutter       = 1
EiraEraseCutterLevel     = 10
LagReduction             = 0
HealOwnerBreeze          = 0
SteinWandTelePause       = 0